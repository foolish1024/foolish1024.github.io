...
- 调优
  ...
  ```xml
  -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -Djava.awt.headless=true -XX:CMSInitiatingOccupancyFraction=75 -XX:+UseCMSInitiatingOccupancyOnly -Xmx500m -Xss204
  ```

  `-XX:+UseParNewGC -XX:+UseConcMarkSweepGC`：使用新生代串行收集器+老年代CMS；

  `-Xmx500m`：设置应用程序能够使用的最大内存；

  `-Xss204`m：设置Java线程堆栈大小；

  `-Xmx500m`： 设置最大 Java 堆大小。
